...
double heuristic(int x1, int y1, int x2, int y2, bool button) {
    if (button) {
...
void aStar(const vector<vector<double>>& graph, int startX, int startY,
           int goalX, int goalY, bool button) {
    ...
    Node* start = new Node(startX, startY, 0.0,
                           heuristic(startX, startY, goalX, goalY, button));
    openSet.push(start);
            ...
            if (nx >= 0 && nx < ROW && ny >= 0 && ny < COL &&
                ...
                if (nodes[nx][ny] == nullptr || newDist < nodes[nx][ny]->g) {
                    if (nodes[nx][ny] == nullptr) {
                        nodes[nx][ny] = new Node(
                            nx, ny, newDist,
                            heuristic(nx, ny, goalX, goalY, button), current);
...
int main() {
    ...
    aStar(graph, startX, startY, goalX, goalY, true);
    ...
    startTimeVal = clock();
    aStar(graph, startX, startY, goalX, goalY, false);
    finishTimeVal = clock();
    cout << "dikstra take "
         << (double)(finishTimeVal - startTimeVal) / CLOCKS_PER_SEC;
